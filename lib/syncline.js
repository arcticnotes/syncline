...
const STATE_NAMES = [];
STATE_NAMES[ STATE_READY            ] = 'READY';
STATE_NAMES[ STATE_INPUT_INCOMPLETE ] = 'INPUT_INCOMPLETE';
STATE_NAMES[ STATE_INPUT_DRAINED    ] = 'INPUT_DRAINED';
STATE_NAMES[ STATE_INPUT_COMPLETE   ] = 'INPUT_COMPLETE';
STATE_NAMES[ STATE_CANCELED         ] = 'CANCELED';
STATE_NAMES[ STATE_ERROR            ] = 'ERROR';
STATE_NAMES[ STATE_OUTPUT_INCOMPLETE] = 'OUTPUT_INCOMPLETE';
STATE_NAMES[ STATE_OUTPUT_DRAINED   ] = 'OUTPUT_DRAINED';
STATE_NAMES[ STATE_TERMINATING      ] = 'TERMINATING';
...
const TRACE_LEVEL_PROTOCOL = 1;
const TRACE_LEVEL_IO       = 2;
const TRACE_LEVEL_ALL      = 3;
...
export class Syncline extends EventEmitter {
	...
	static async spawn( command, args = [], options = {}) {
		...
		syncline.#traceLevel = options.trace || TRACE_LEVEL_OFF;
		...
		await new Promise( ( resolveInit, rejectInit) => {
			...
			syncline.#worker.on( 'message', ( [ type, ...parameters]) => {
				switch( type) {
					case MESSAGE_READY:
						if( syncline.#traceLevel >= TRACE_LEVEL_PROTOCOL)
							console.log( `${ performance.now()}: main: received message: ${ type}`);
						resolveInit();
					...
					default:
						if( syncline.#traceLevel >= TRACE_LEVEL_PROTOCOL)
							console.log( `${ performance.now()}: main: received unknown message (bug): ${ type}`);
	...
	exchange( inputLine, timeout = TIMEOUT_EXCHANGE) {
		...
		// step 1. send input
		let remainingInput = inputLine;
		while( true) {
			const { read, written} = new TextEncoder().encodeInto( remainingInput, this.#data);
			this.#control[ INDEX_LENGTH] = written;
			remainingInput = remainingInput.substring( read);
			if( !remainingInput)
				break;
			if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
				console.log( `${ performance.now()}: main: ${ written}B written, setting state=${ STATE_NAMES[ STATE_INPUT_INCOMPLETE]}`);
			...
			switch( inputState) {
				case STATE_INPUT_DRAINED:
					if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
						console.log( `${ performance.now()}: main: state changed to ${ STATE_NAMES[ inputState]}`);
					break; // do nothing, loop back to send the next chunk
				case STATE_ERROR:
		...
		output: while( true) {
			...
			const length = this.#control[ INDEX_LENGTH];
			switch( outputState) {
				case STATE_OUTPUT_INCOMPLETE:
					if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
						console.log( `${ performance.now()}: main: state changed to ${ STATE_NAMES[ outputState]}, reading ${ length}B, setting state=${ STATE_NAMES[ STATE_OUTPUT_DRAINED]}`);
				...
				case STATE_ERROR:
					...
					throw new Error( new TextDecoder( 'utf-8').decode( this.#data.slice( 0, length)));
				default:
					if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
						console.log( `${ performance.now()}: main: state unexpectedly changed to ${ STATE_NAMES[ outputState]} (bug)`);
	...
	async close() {
		return await new Promise( ( resolveTermination, rejectTermination) => {
			if( this.#terminated) {
				if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
					console.log( `${ performance.now()}: main: worker already terminated, resolving immediately`);
				resolveTermination( this.#terminated);
			} else {
			...
			switch( state) {
				case STATE_TERMINATING:
					if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
						console.log( `${ performance.now()}: main: worker being terminated, waiting`);
					break; // termination initiated by another call? just wait
				case STATE_READY:
				case STATE_ERROR:
					if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
						console.log( `${ performance.now()}: main: terminating worker by setting state to ${ STATE_NAMES[ STATE_TERMINATING]}`);
...
export class SynclineWorker {
	...
	static async spawn( workerData, parentPort) {
		...
		worker.#traceLevel = workerData.options.trace;
	...
	#setError( error) {
		if( this.#errorMessage === undefined)
			try {
				...
				if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
					console.log( `${ performance.now()}: worker: setting error: ${ this.#errorMessage}`);
				this.#parentPort.postMessage( [ MESSAGE_ERROR, this.#errorMessage]);
			}
	}

	#ingestStdout( chunk) {
		if( this.#traceLevel >= TRACE_LEVEL_IO)
			console.log( `${ performance.now()}: worker: ${ chunk.length}B arrived on stdout`);
		if( this.#errorMessage !== undefined)
		...
		for( let indexLF; ( indexLF = remaining.indexOf( 0x0A)) >= 0;) {
			...
			const concat = Buffer.concat( this.#stdoutBuffer);
			if( this.#traceLevel >= TRACE_LEVEL_IO)
				console.log( `${ performance.now()}: worker: constructing a line from ${ concat.length}B from stdout`);
			try {
				decodedLine = concat.toString( 'utf-8');
			} catch( error) {
			...
			if( this.#dropLine) {
				this.#dropLine--;
				if( this.#traceLevel >= TRACE_LEVEL_IO)
					console.log( `${ performance.now()}: worker: dropping a line to compensate previous timeout`);
	...
	#ingestStderr( chunk) {
		if( this.#traceLevel >= TRACE_LEVEL_IO)
			console.log( `${ performance.now()}: worker: ${ chunk.length}B arrived on stderr`);
		if( this.#errorMessage !== undefined)
			return; // once an error occurs, all stdout/stderr data is dropped
		try {
			let remaining = chunk;
			for( let indexLF; ( indexLF = remaining.indexOf( 0x0A)) >= 0;) {
				...
				remaining = remaining.slice( indexLF + 1);
				const concat = Buffer.concat( this.#stderrBuffer);
				if( this.#traceLevel >= TRACE_LEVEL_IO)
					console.log( `${ performance.now()}: worker: constructing a line from ${ concat.length}B from stderr`);
				const line = concat.toString( 'utf-8');
	...
	async run() {
		let state;
		for( ; ( state = Atomics.load( this.#control, INDEX_STATE)) !== STATE_TERMINATING;)
			switch( state) {
				case STATE_READY:
				case STATE_ERROR:
					await new Promise( resolve => setImmediate( resolve)); // yield the thread for IO
					if( this.#traceLevel >= TRACE_LEVEL_ALL)
						console.log( `${ performance.now()}: worker: idle, parking for ${ WORKER_PARK}ms`);
		...
		if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
			console.log( `${ performance.now()}: worker: state changed to ${ STATE_NAMES[ state]}, closing stdin`);
		this.#stdin.end();
	}
	...
	async #handleInputIncomplete() {
		if( this.#errorMessage !== undefined) {
			if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
				console.log( `${ performance.now()}: worker: state changed to ${ STATE_NAMES[ STATE_INPUT_INCOMPLETE]}, setting to ${ STATE_NAMES[ STATE_ERROR]}`);
		...
		const length = this.#control[ INDEX_LENGTH];
		if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
			console.log( `${ performance.now()}: worker: state changed to ${ STATE_NAMES[ STATE_INPUT_INCOMPLETE]}, transferring ${ length}B, and setting to ${ STATE_NAMES[ STATE_INPUT_DRAINED]}`);
		this.#stdin.write( Buffer.from( this.#shared, OFFSET_DATA, length));
	...
	async #handleInputComplete() {
		if( this.#errorMessage !== undefined) {
			if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
				console.log( `${ performance.now()}: worker: state changed to ${ STATE_NAMES[ STATE_INPUT_COMPLETE]}, setting to ${ STATE_NAMES[ STATE_ERROR]}`);
			this.#control[ INDEX_LENGTH] = new TextEncoder().encodeInto( this.#errorMessage, this.#data).written;
			Atomics.store( this.#control, INDEX_STATE, STATE_ERROR);
			Atomics.notify( this.#control, INDEX_STATE, 1);
			return;
		...
		const length = this.#control[ INDEX_LENGTH];
		if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
			console.log( `${ performance.now()}: worker: state changed to ${ STATE_NAMES[ STATE_INPUT_COMPLETE]}, transferring ${ length}B followed by LF, and waiting for stdout`);
		this.#stdin.write( Buffer.from( this.#shared, OFFSET_DATA, length));
		this.#stdin.write( LF);
		...
		if( errorMessage !== undefined) {
			if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
				console.log( `${ performance.now()}: worker: stdout failed, setting state to ${ STATE_NAMES[ STATE_ERROR]}`);
		...
		output: for( let remaining = outputLine;;) {
			...
			if( !remaining) {
				if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
					console.log( `${ performance.now()}: worker: ${ written}B written, setting state to ${ STATE_NAMES[ STATE_READY]}`);
				Atomics.store( this.#control, INDEX_STATE, STATE_READY);
				Atomics.notify( this.#control, INDEX_STATE, 1);
				break output;
			}
			if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
				console.log( `${ performance.now()}: worker: ${ written}B written, setting state to ${ STATE_NAMES[ STATE_OUTPUT_INCOMPLETE]}`);
			...
			switch( state) {
				case STATE_CANCELED:
					if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
						console.log( `${ performance.now()}: worker: state changed to ${ STATE_NAMES[ STATE_CANCELED]}, setting to ${ STATE_NAMES[ STATE_ERROR]}`);
					Atomics.store( this.#control, INDEX_STATE, STATE_ERROR);
					Atomics.notify( this.#control, INDEX_STATE, 1);
					break output;
				case STATE_OUTPUT_DRAINED:
					if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
						console.log( `${ performance.now()}: worker: state changed to ${ STATE_NAMES[ STATE_OUTPUT_DRAINED]}, continuing next chunk`);
					continue output;
				default:
					if( this.#traceLevel >= TRACE_LEVEL_PROTOCOL)
						console.log( `${ performance.now()}: worker: state unexpectedly changed to ${ STATE_NAMES[ state]} (bug)`);
					this.#setError( new Error( `unexpected worker state: ${ state}`)); // bug
			}
		}
	}
